...
<head>
    ...
    <style>
        ...
        .stars-wrapper { position: fixed; top: 0; left: 0; width: 100%; height: 100%; z-index: -1; overflow: hidden; }
        ...
        #stars:after {
            ...
            position: absolute;
            top: 2000px;
            width: 1px; height: 1px; background: transparent; box-shadow: inherit;
        }
        ...
        #stars2:after {
            ...
            position: absolute;
            top: 2000px;
            width: 2px; height: 2px; background: transparent; box-shadow: inherit;
        }
        ...
        #stars3:after {
            ...
            position: absolute;
            top: 2000px;
            width: 3px; height: 3px; background: transparent; box-shadow: inherit;
        }
        ...
        h1 { margin: 0; color: white; font-weight: 900; text-transform: uppercase; letter-spacing: 2px; }

        .btn-back { 
            background: rgba(255,255,255,0.15); color: white; padding: 10px 25px; 
            text-decoration: none; border-radius: 20px; font-weight: 900; border: 1px solid white; transition: 0.2s;
        }

        .layout-grid { display: grid; grid-template-columns: 3fr 2fr; gap: 40px; }

        /* CALENDARIO CRISTAL TRANSPARENTE */
        .calendar-card { 
        ...
        .calendar-header h2 { margin: 0; color: var(--accent-gold); font-size: 2rem; text-transform: capitalize; }
        
        .btn-nav-month {
            background: rgba(255,255,255,0.1); border: none; padding: 10px 15px; border-radius: 12px; cursor: pointer;
            font-weight: 900; color: white; transition: 0.2s;
        }

        .days-grid { display: grid; grid-template-columns: repeat(7, 1fr); gap: 12px; text-align: center; }
        .day-name { font-weight: 900; color: white; margin-bottom: 10px; font-size: 1.1rem; }
        
        /* CELDAS CALENDARIO */
        .day-cell { 
            aspect-ratio: 1; min-height: 50px; display: flex; justify-content: center; align-items: center; 
            border-radius: 15px; border: 1px solid rgba(255,255,255,0.1); background: rgba(255,255,255,0.05);
            cursor: pointer; transition: all 0.2s; font-weight: 900; font-size: 1.3rem; color: white;
            position: relative;
        }
        .day-cell:hover:not(.empty) { background: rgba(255,255,255,0.2); border-color: var(--accent-gold); transform: scale(1.05); }
        .day-cell.empty { background: transparent; border: none; cursor: default; }
        
        /* DISEÑO RECUPERADO: Marrón con Check Blanco */
        .day-cell.my-vote { 
            background: var(--wood-selected) !important; color: white !important; border-color: var(--wood-brown) !important;
            box-shadow: 0 0 15px rgba(197, 160, 89, 0.4);
        }
        .day-cell.my-vote::after {
            content: '✓'; position: absolute; top: 5px; right: 8px; font-size: 0.9rem; color: white; font-weight: 900;
        }
        ...
        .btn-save {
            ...
            color: white; border: none; border-radius: 20px; font-weight: 900; font-size: 1.2rem;
            text-transform: uppercase; cursor: pointer; transition: 0.2s ease;
        }
        .btn-save:hover { background: var(--purple-light); }

        .ranking-card { 
            background: rgba(0,0,0,0.6); backdrop-filter: blur(12px); border-radius: 25px; padding: 40px 30px; 
    ...
    <div class="layout-grid">
        <div class="calendar-card">
            <div class="calendar-header">
                <button class="btn-nav-month" onclick="cambiarMes(-1)">◀</button>
                <h2 id="monthDisplay">Abril 2026</h2>
                <button class="btn-nav-month" onclick="cambiarMes(1)">▶</button>
            </div>
            
            <div class="days-grid">
                <div class="day-name">Lu</div><div class="day-name">Ma</div><div class="day-name">Mi</div><div class="day-name">Ju</div><div class="day-name">Vi</div><div class="day-name">Sá</div><div class="day-name">Do</div>
            </div>
            
            <div class="days-grid" id="calendarGrid"></div>
            
            <form method="POST">
                <input type="hidden" name="fechas_seleccionadas" id="inputFechas">
                <button type="submit" name="guardar_fechas" class="btn-save">🏕️✨ Guardar mi disponibilidad 🍻🏠</button>
            </form>
        </div>

        <div class="ranking-card">
            <h3>🏆 Más votadas</h3>
            <?php if (empty($ranking_fechas)): ?>
                <p style="opacity: 0.6; font-style: italic;">Nadie ha votado todavía.</p>
            <?php else: ?>
                <?php foreach($ranking_fechas as $rf): ?>
                    <div class="rank-item">
                        <strong>🗓️ <?= date('d/m/Y', strtotime($rf['fecha'])) ?></strong>
                        <span style="float:right; color:var(--accent-gold); font-weight:900;"><?= $rf['total_votos'] ?> v.</span>
                    </div>
                <?php endforeach; ?>
            <?php endif; ?>
        </div>
...
<script>
    let seleccionadas = <?= json_encode($mis_votos) ?>;
    let fechaVisual = new Date(2026, 3, 1); 
    ...
    function renderizarCalendario() {
        ...
        monthDisplay.innerText = mesesNombres[mes] + " " + anio;

        const primerDia = new Date(anio, mes, 1);
        let diaSemanaEmpieza = primerDia.getDay() - 1; 
        if (diaSemanaEmpieza === -1) diaSemanaEmpieza = 6;

        for (let i = 0; i < diaSemanaEmpieza; i++) {
        ...
        for (let dia = 1; dia <= ultimoDia; dia++) {
            ...
            if (seleccionadas.includes(fechaStr)) div.classList.add('my-vote');

            div.onclick = function() {
                if (seleccionadas.includes(fechaStr)) {
                    seleccionadas = seleccionadas.filter(f => f !== fechaStr);
                    this.classList.remove('my-vote');
                } else {
                    seleccionadas.push(fechaStr);
                    this.classList.add('my-vote');
                }
                document.getElementById('inputFechas').value = seleccionadas.join(',');
            };
            grid.appendChild(div);
        }
        document.getElementById('inputFechas').value = seleccionadas.join(',');
